...
const testConnection = async() => {
    try{
        ...
        console.log(`PostgreSQL verison : ${versionNumber}`);
        console.log(`Serveer time: ${result.rows[0].current_time}`)
        return true;
    } catch(error) {
...
const query = async(text, params) => {
    ...
    try{
        ...
        console.error('Databae query error:', error.message);
...
process.on('SIGNIT' ,async() => {
    console.log('\n Recieved SIGNIT, closing database connections...');
    await closePool();
    process.exit(0);
});

process.on('SIGTERM' ,async() => {
    console.log('\n Recieved SIGTERM, closing database connections...');
    await closePool();
    process.exit(0);
});
